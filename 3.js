...
function hireGroup(groups, budget) {
  ...
  if (budget !== undefined) {
    for (let i = 0; i < groups.length; i++) {
      ...
      let groupPrice = groups[i]["price"];

      switch (groupName) {
        case "Twoosh":
          if (budget > groupPrice) {
            countTwoosh++;
            budget -= groupPrice;
          }
          break;
    ...
    countTotal = countTwoosh + countItsy + countGET7 + countWhitepink + countJYV
    console.log(`${countTwoosh}, ${countItsy}, ${countGET7}, ${countWhitepink}, ${countJYV}`)
  } else {
